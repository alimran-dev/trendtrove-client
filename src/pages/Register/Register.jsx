      ...
      if (/^.{6,}$/.test(password)) {
        if (/.*[A-Z].*/.test(password)) {
          if (/.*[\W_].*/.test(password)) {
            ...
            setError("Password must have minimum 1 special characters.");
          }
        } else {
          setError("Password must have minimum 1 capital letter.");
        }
      } else {
        setError("Password must be minimum 6 characters.");
      }
    } else {
      setError("You must accept our terms and conditions to register.");
    }
  };
  const handleGoogle = () => {
    setError(null);
        ...
        <div className="relative flex flex-col text-gray-700 bg-white shadow-md md:w-96 rounded-xl bg-clip-border mx-auto my-20">
          ...
          <div className="p-6 pt-0">
            ...
            <button
              ...
              className="flex items-center justify-center gap-1.5 w-full border-2 border-pink-600 py-1.5 my-2 rounded-lg hover:cursor-pointer hover:shadow-md hover:shadow-pink-500/40"
            >
